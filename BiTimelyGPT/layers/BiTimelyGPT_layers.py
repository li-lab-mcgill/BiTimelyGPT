...
class RetentionBlock(nn.Module):
    ...
    def __init__(self, config: RetNetConfig,
        ...
        self.ffn = nn.Sequential(
            nn.Linear(config.d_model, config.d_ff, bias=config.use_bias_in_mlp),
            nn.GELU(),
            nn.Linear(config.d_ff, config.d_model, bias=config.use_bias_in_mlp),
        )
